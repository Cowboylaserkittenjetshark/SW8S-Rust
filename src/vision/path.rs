...
use opencv::{
    core::{in_range, Size, VecN, Vector, AlgorithmHint},
    imgcodecs::imwrite,
...
impl VisualDetector<i32> for Path {
    ...
    fn detect(
        ...
        cvt_color(&self.image.0, &mut yuv_image, COLOR_RGB2YUV, 0, AlgorithmHint::ALGO_HINT_DEFAULT).unwrap();
        ...
        cvt_color(&yuv_image, &mut self.image.0, COLOR_YUV2RGB, 0, AlgorithmHint::ALGO_HINT_DEFAULT).unwrap();
...
impl VisualDetector<f64> for Path {
    ...
    fn detect(
        ...
        cvt_color(&image, &mut yuv_image, COLOR_RGB2YUV, 0, AlgorithmHint::ALGO_HINT_DEFAULT).unwrap();
        yuv_image = kmeans(&yuv_image, self.num_regions, self.attempts);
        let image_center = ((yuv_image.cols() / 2) as f64, (yuv_image.rows() / 2) as f64);
